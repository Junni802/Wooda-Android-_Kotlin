...
import com.example.wooda.databinding.ActivityKaKaoMapBinding
...
class KaKaoMapActivity : AppCompatActivity() {
    companion object {
        ...
        const val API_KEY = "KakaoAK 00eb38663ba059d81c68c3cc37bcc0dc"  // REST API 키
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listAdapter.setItemClickListener(object: ListAdapter.OnItemClickListener {
            override fun onClick(v: View, position: Int) {
                ...
                Log.d("위도 여", "${listItems[position].x}")
                Log.d("경도 여", "${listItems[position].y}")
                Log.d("이름 여", "${listItems[position].name}")
                x = listItems[position].x.toString()
                y = listItems[position].y.toString()
                name = listItems[position].name.toString()
            }
        })
